    ...
    <div className="flex flex-col items-center  py-16 bg-[#172022] gap-6 sm:px-12 lg:px-32">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 w-full">
        {upcomingSessions.map((session, index) => (
          <div
            key={index}
            className=" shadow-md text-white py-8 px-6  lg:px-2  flex lg:flex-row flex-col lg:items-center justify-between gap-4   hover:shadow-lg transition-all duration-200 ease-in lg:h-32 border-y border-[#4F5152] hover:bg-gradient-to-r hover:from-transparent hover:via-[#233032]/80 hover:to-transparent"
          >
            <div className="flex items-center gap-4 md:gap-12">
              <p className="hidden lg:block text-sm text-[#70CCE2]  font-semibold">{session.time}</p>
              <img
                src={session.image}
                alt={session.artist}
              ...
              <div className="text-start ">
                <p className="font-bold text-sm md:text-base lg:text-lg">{session.artist}</p>
                <span className="text-sm text-gray-400">{session.role}</span>
                <p className="block lg:hidden text-sm text-[#70CCE2]  font-semibold">{session.time}</p>
              </div>
            </div>
